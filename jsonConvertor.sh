...
echo "ip_prefix : [" >> awsJsonFormat.json
n_id=1
n_city=1
n_place=1
n_external_ip=1;
...
do
   ...
   if [[ ! -z $id  &&   $n_id == 1 ]];then
    ...
    if [[  $n_id != 1 &&  $n_city != 1 &&  $n_place != 1 &&   $n_external_ip != 1 ]];then
        ...
        echo "\"id\":\"$n_id\",">> awsJsonFormat.json
        echo "\"city\":\"$n_city\",">> awsJsonFormat.json
        echo "\"place\":\"$n_place\",">> awsJsonFormat.json
        echo "\"external_ip\":\"$n_external_ip\"">> awsJsonFormat.json
        echo "}," >> awsJsonFormat.json
        n_id=1
        n_city=1
        n_place=1
